...
export function gerarRelatorioConsultor(casal, pctEsposo, pctEsposa) {
  ...
  // === RULE ENGINE (MOTOR DE REGRAS DINÂMICO) ===
  const dimensoes = [...BLOCOS.bloco1, ...BLOCOS.bloco2]
  const linguagens = BLOCOS.linguagens

  let somaDiv = 0
  let maiorDivergencia = { key: dimensoes[0], valor: -1, nome: NOMES[dimensoes[0]], esposo: 0, esposa: 0 }

  // Calcula divergências e define tema central
  const dadosDimensoes = dimensoes.map(key => {
    const div = Math.abs(pctEsposo[key] - pctEsposa[key])
    somaDiv += div
    if (div > maiorDivergencia.valor) {
      maiorDivergencia = { key, valor: div, nome: NOMES[key], esposo: pctEsposo[key], esposa: pctEsposa[key] }
    }
  ...
  const divBadge = (key) => key === maiorDivergencia.key ? ' <span style="background:#D32F2F;color:#fff;padding:2px 5px;border-radius:3px;font-size:10px;margin-left:8px;font-weight:bold">DIV</span>' : ''

  // === CONDICIONAIS DE TEXTO ===
  const menosDivNome = maiorDivergencia.esposo < maiorDivergencia.esposa ? casal.nome_esposo : casal.nome_esposa
  const maisDivNome = maiorDivergencia.esposo < maiorDivergencia.esposa ? casal.nome_esposa : casal.nome_esposo

  let protocoloExtra = ''
  if (pctEsposo.financeiro < 60 || pctEsposa.financeiro < 60) {
    const liderFin = pctEsposo.financeiro >= pctEsposa.financeiro ? casal.nome_esposo : casal.nome_esposa
    protocoloExtra = `<li><strong>Planejamento financeiro mensal conjunto:</strong> ${liderFin.split(' ')[0]} lidera 30 min por mês.</li>`
  } else if (pctEsposo.sinergia < 60 || pctEsposa.sinergia < 60) {
    protocoloExtra = `<li><strong>Resgate de Parceria Prática:</strong> Definir 1 tarefa doméstica colaborativa sem divisão rígida, para resgatar a percepção de equipe.</li>`
  } else {
    protocoloExtra = `<li><strong>Revisão de Metas Conjuntas:</strong> Manter alinhamento trimestral para garantir que a Sinergia continue forte.</li>`
  }

  // === RENDERIZAÇÃO DA TABELA HTML ===
  const tableRowsLinguagens = dadosLinguagens.map(d => `
    <tr>
      <td style="padding:10px 14px;border-bottom:1px solid #eee;color:#333;font-size:13px">${d.nome}</td>
      <td style="padding:10px 14px;border-bottom:1px solid #eee;color:#6A1B9A;font-weight:bold;font-size:13px">${d.esposa}%</td>
      <td style="padding:10px 14px;border-bottom:1px solid #eee;color:#1565C0;font-weight:bold;font-size:13px">${d.esposo}%</td>
    </tr>
  `).join('')

  const tableRowsDimensoes = dadosDimensoes.map(d => `
    <tr>
      <td style="padding:10px 14px;border-bottom:1px solid #eee;color:#333;font-size:13px">${d.nome} — ${d.div.toFixed(2)}%${divBadge(d.key)}</td>
      <td style="padding:10px 14px;border-bottom:1px solid #eee;color:#6A1B9A;font-size:13px">${d.esposa}%${alertIcon(d.esposa)}</td>
      <td style="padding:10px 14px;border-bottom:1px solid #eee;color:#1565C0;font-size:13px">${d.esposo}%${alertIcon(d.esposo)}</td>
    </tr>
  `).join('')
...
<head>
<meta charset="UTF-8">
<title>Roteiro de Devolutiva — ${casal.nome_esposo} & ${casal.nome_esposa}</title>
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  body{font-family:'Arial',sans-serif;background:#fff;color:#111;padding:40px}
  .container{max-width:800px;margin:0 auto}
  .header-box{border:2px solid #222;padding:24px;margin-bottom:30px;border-radius:4px}
  h1{font-size:22px;margin-bottom:16px;text-transform:uppercase;letter-spacing:1px;border-bottom:2px solid #222;padding-bottom:10px}
  h2{font-size:16px;margin:24px 0 12px 0;color:#333;background:#f5f5f5;padding:8px 12px;border-left:4px solid #C9A84C}
  p{font-size:14px;line-height:1.6;margin-bottom:12px;color:#444}
  .highlight{background:#FFF9C4;padding:2px 4px;font-weight:bold}
  table{width:100%;border-collapse:collapse;margin-bottom:20px}
  th{background:#f9f9f9;text-align:left;padding:12px 14px;font-size:12px;text-transform:uppercase;color:#555;border-bottom:2px solid #ddd}
</style>
</head>
<body>
<div class="container">
  ...
  <div class="header-box">
    <h1 style="border:none;margin:0;padding:0">ROTEIRO DE DEVOLUTIVA — SESSÃO ONLINE</h1>
    <p style="margin:8px 0 16px 0;font-size:13px;color:#666">Perfil 4D — Análise Comportamental & Linguagens do Amor</p>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:16px">
      <div style="background:#f5f0fb;padding:12px;border-radius:4px;border-left:3px solid #6A1B9A">
        <strong style="color:#6A1B9A">${casal.nome_esposa}</strong><br>
        <span style="font-size:13px">Média Geral: ${(dimensoes.reduce((a, b) => a + pctEsposa[b], 0) / dimensoes.length).toFixed(2)}%</span>
      </div>
      <div style="background:#f0f5fc;padding:12px;border-radius:4px;border-left:3px solid #1565C0">
        <strong style="color:#1565C0">${casal.nome_esposo}</strong><br>
        <span style="font-size:13px">Média Geral: ${(dimensoes.reduce((a, b) => a + pctEsposo[b], 0) / dimensoes.length).toFixed(2)}%</span>
      </div>
    </div>
    <div style="font-size:15px;background:#f9f9f9;padding:12px;border:1px solid #eee">
      <strong>Diagnóstico:</strong> <span class="highlight">${diagnostico}</span> &nbsp;|&nbsp; 
      <strong>Divergência Média:</strong> ${divergenciaMedia}% &nbsp;|&nbsp; 
      <strong>Tema Central:</strong> ${maiorDivergencia.nome}
    </div>
  </div>

  <div style="background:#fff3e0;border:1px solid #ffe0b2;padding:20px;border-radius:4px;margin-bottom:30px">
    <h3 style="color:#e65100;font-size:14px;margin-bottom:10px;text-transform:uppercase">🎯 Cola Rápida — Consulta Durante a Sessão</h3>
    <p style="margin-bottom:16px">
      <strong>TEMA CENTRAL: ${maiorDivergencia.nome} (${maiorDivergencia.valor.toFixed(2)}% de div.)</strong><br>
      A maior divergência absoluta do casal. Quando <strong>${menosDivNome.split(' ')[0]}</strong> pontua muito baixo aqui (${Math.min(maiorDivergencia.esposo, maiorDivergencia.esposa)}%), cria-se um bloqueio que reflete em outras áreas afetivas, forçando <strong>${maisDivNome.split(' ')[0]}</strong> a carregar o peso estrutural dessa dimensão.
    </p>
    <ul style="font-size:14px;line-height:1.6;margin-left:20px;color:#444">
      <li><strong>Protocolo de reconexão de ${maiorDivergencia.nome}:</strong> ${menosDivNome.split(' ')[0]} pratica a exposição progressiva; ${maisDivNome.split(' ')[0]} pratica a escuta ativa sem cobrança imediata.</li>
      <li><strong>Expressividade Diária:</strong> 1 gesto afetivo por dia para manter o canal de intimidade quente independente das crises.</li>
      ${protocoloExtra}
    </ul>
  </div>

  <h2>TELA ÚNICA — Compartilhar nas Fases 2 e 3</h2>
  <p style="font-size:12px;color:#666">Abra esta página e compartilhe do início da Fase 2 até o fim da Fase 3.</p>

  <table style="margin-top:20px">
    <thead>
      <tr>
        <th colspan="3" style="background:#e8e0d4;color:#0D1B3E;font-size:14px">Bloco A — As 5 Linguagens do Amor</th>
      </tr>
      <tr>
        <th>Linguagem</th>
        <th>${casal.nome_esposa.split(' ')[0]}</th>
        <th>${casal.nome_esposo.split(' ')[0]}</th>
      </tr>
    </thead>
    <tbody>
      ${tableRowsLinguagens}
      <tr><td colspan="3" style="font-size:11px;color:#888;padding:8px 14px">*M = Dominante Mulher | *H = Dominante Homem</td></tr>
    </tbody>
  </table>
  ...
  <table>
    <thead>
      <tr>
        <th colspan="3" style="background:#e8e0d4;color:#0D1B3E;font-size:14px">Bloco B — Dimensões-Chave da Sessão</th>
      </tr>
      <tr>
        <th>Dimensão</th>
        <th>${casal.nome_esposa.split(' ')[0]}</th>
    ...
    <tbody>
      ${tableRowsDimensoes}
      <tr><td colspan="3" style="font-size:11px;color:#888;padding:8px 14px">⚠️ = Atenção Crítica (Abaixo de 50%) | DIV = Maior Divergência Absoluta</td></tr>
    </tbody>
  </table>

  <h2>Fase de Encerramento (Dica para o Analista)</h2>
  <p><strong>Frase de transição sugerida:</strong> "${casal.nome_esposa.split(' ')[0]} e ${casal.nome_esposo.split(' ')[0]}, vocês têm pontos de conexão reais. O que esse mapa revela não é incompatibilidade, mas sim que o desequilíbrio em <strong>${maiorDivergencia.nome}</strong> está bloqueando o afeto de chegar até a outra ponta. Trabalhando este ponto focal com os protocolos práticos, as demais áreas fluirão com muito mais leveza."</p>
  
  <div style="margin-top:40px;font-size:11px;color:#aaa;text-align:center;border-top:1px solid #eee;padding-top:20px">
    Perfil 4D | Roteiro Dinâmico | ${funcaoProfissional} ${nomeProfissional} | © 2026
  </div>
